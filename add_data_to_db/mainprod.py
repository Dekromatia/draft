import os
os.chdir('D:/Mestrado/COURSE 2/Diploma/draft/add_data_to_db')
# import psycopg2
...
conn = mysql.connector.connect(
    host="localhost",
#     host_password="3281#Is-77" 31.31.198.106,
    database="u2066421_default",
    user="u2066421_default",
    password="3281#Df-77")
...
with open('./tables_csv/Site.csv', 'r', encoding="UTF-8") as f_site:
    next(f_site)  # skip header row
    cur.execute("LOAD DATA INFILE 'tables_csv/Site.csv' INTO TABLE site "
...
with open('./tables_csv/Manufact.csv', 'r', encoding="UTF-8") as f_m:
    next(f_m)  # skip header row
    cur.execute("LOAD DATA INFILE 'tables_csv/Manufact.csv' INTO TABLE manufact "
            "FIELDS TERMINATED BY ';' "
            "IGNORE 1 ROWS")
...
with open('./tables_csv/Artifact.csv', 'r', encoding="UTF-8") as f_m:
    next(f_m)  # skip header row
    cur.execute("LOAD DATA INFILE 'tables_csv/Artifact.csv' INTO TABLE manufact "
            "FIELDS TERMINATED BY ';' "
            "IGNORE 1 ROWS")
...
with open('./tables_csv/Stamp.csv', 'r', encoding="UTF-8") as f_m:
    next(f_m)  # skip header row
    cur.execute("LOAD DATA INFILE 'tables_csv/Stamp.csv' INTO TABLE manufact "
            "FIELDS TERMINATED BY ';' "
            "IGNORE 1 ROWS")
...
with open('./tables_csv/3DModel.csv', 'r', encoding="UTF-8") as f_m:
    next(f_m)  # skip header row
    cur.execute("LOAD DATA INFILE 'tables_csv/3DModel.csv' INTO TABLE manufact "
            "FIELDS TERMINATED BY ';' "
            "IGNORE 1 ROWS")
